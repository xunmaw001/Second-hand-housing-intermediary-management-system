...
//            chuzuLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<ChuzuLiuyanEntity> queryWrapper = new EntityWrapper<ChuzuLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        ChuzuLiuyanEntity chuzuLiuyanEntity = chuzuLiuyanService.selectOne(queryWrapper);
        chuzuLiuyan.setUpdateTime(new Date());
        if(chuzuLiuyanEntity==null){
            //  String role = String.valueOf(request.getSession().getAttribute("role"));
            //  if("".equals(role)){
            //      chuzuLiuyan.set
            //  }
    ...
    */
    ...
    public R save( String fileName){
        ...
        try {
            List<ChuzuLiuyanEntity> chuzuLiuyanList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            ChuzuLiuyanEntity chuzuLiuyanEntity = new ChuzuLiuyanEntity();
//                            chuzuLiuyanEntity.setChuzuId(Integer.valueOf(data.get(0)));   //出租 要改的
//                            chuzuLiuyanEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            chuzuLiuyanEntity.setChuzuLiuyanText(data.get(0));                    //留言内容 要改的
//                            chuzuLiuyanEntity.setReplyText(data.get(0));                    //回复内容 要改的
//                            chuzuLiuyanEntity.setInsertTime(date);//时间
//                            chuzuLiuyanEntity.setUpdateTime(new Date(data.get(0)));          //回复时间 要改的
//                            chuzuLiuyanEntity.setCreateTime(date);//时间
                            chuzuLiuyanList.add(chuzuLiuyanEntity);
                        ...
                        //查询是否重复
                        chuzuLiuyanService.insertBatch(chuzuLiuyanList);
                        return R.ok();
                    }
                }
            }
